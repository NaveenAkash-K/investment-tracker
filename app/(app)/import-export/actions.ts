...
export async function restoreFullBackup(formData: FormData) {
    ...
    if (record.format !== "investment-tracker-backup" || ![1, 2, 3, 4, 5, 6].includes(Number(record.version)) || !record.data) {
        throw new Error("This is not a supported Investment Tracker backup.");
    }

    const { error } = await supabase.rpc("restore_complete_portfolio_backup_v12", { p_backup: backup });
...
export async function importSipPlans(formData: FormData) {
    ...
    const sipRows = rows.map((row, index) => {
        ...
        if (monthlyAmount < 0 || !Number.isInteger(monthlyAmount)) {
            throw new Error(`Row ${index + 2}: monthly_amount must be a non-negative whole rupee amount.`);
        }
