...
type EventRow = {
    ...
    rule_evidence: unknown;
};
type Impact = { event_id: string; market_key: string; direction: number | string; magnitude: number | string; confidence: number | string; reason: string; origin: string; rule_keys: unknown };
...
function ruleEvidence(value: unknown) {
    if (!Array.isArray(value)) return [];
    return value.flatMap((item) => {
        if (!item || typeof item !== "object") return [];
        const record = item as Record<string, unknown>;
        const key = typeof record.rule_key === "string" ? record.rule_key : "";
        return key ? [{ key, terms: list(record.matched_terms) }] : [];
    });
}
...
export default async function NewsEventsPage({ searchParams }: { searchParams: SearchParams }) {
    ...
    const [impactsResult, portfolioResult, reactionsResult, linksResult] = await Promise.all([
        ids.length ? supabase.from("market_event_impacts").select("event_id,market_key,direction,magnitude,confidence,reason,origin,rule_keys").eq("user_id", user.id).in("event_id", ids) : Promise.resolve(empty),
    ...
    const now = new Date();
    const runAgeHours = run ? (now.getTime() - new Date(run.as_of).getTime()) / 3_600_000 : Number.POSITIVE_INFINITY;
    const newsHeartbeatMissed = !run || !Number.isFinite(runAgeHours) || runAgeHours > 3;
    ...
    return <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        {run ? <AnalyzerContractStatus version={run.contract_version} publisher="News worker" /> : null}
        {newsHeartbeatMissed ? <div role="alert" className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900"><p className="font-semibold">News worker heartbeat is stale</p><p className="mt-1">The hourly worker has not published within three hours. Check the GitHub Action and Supabase publication step.</p></div> : run?.status === "failed" ? <div role="alert" className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">The latest news collection run failed. Headlines and impact checks may be incomplete.</div> : null}
